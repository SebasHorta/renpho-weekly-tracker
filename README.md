...
- **The 1-week chart view plots daily readings, not one dot.** A single selected week has only one weekly average — not a trend. When "Weeks shown" is `1`, the chart switches to that week's individual daily readings (added a full daily dataset embed + a parallel `renderDailyChart()` for this) instead of showing a lone point. The table stays a one-row weekly summary regardless — Goal/Target/deltas are inherently weekly concepts with nothing daily to switch them to. Daily values use `snap_to_scale_lb()` for the same reason as elsewhere: individual readings should match what the scale actually showed.
